-- Migration 151 — fix portal_solicitar_substituicao: campo indisponibilidades correto
--
-- PROBLEMA: migration 101 usa i.data_inicio e i.data_fim, mas a tabela
-- indisponibilidades tem apenas a coluna `data DATE` (uma data por registro).
-- Isso causava: "column i. data_inicio does not exist" ao membro solicitar substituição.
--
-- FIX: substituir o NOT EXISTS que checa data_inicio/data_fim por uma checagem
-- que compara i.data = v_escala_data (uma indisponibilidade por data).
  ...
  IF v_ministerio_id IS NOT NULL THEN
    ...
    LOOP
      INSERT INTO public.notificacoes
        (paroquia_id, destinatario_id, tipo, titulo, mensagem, lida, link_referencia)
      VALUES
