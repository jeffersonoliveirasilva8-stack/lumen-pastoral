-- Migration 114 — Fix coord_aprovar_substituicao: evita violação de unique constraint
--
-- PROBLEMA: a RPC fazia UPDATE SET membro_id = substituto_id na linha original.
-- Se o substituto já tinha uma linha nessa escala+ministério (escalado independentemente
-- ou de uma tentativa anterior), o UPDATE criava uma duplicata em
-- (escala_id, membro_id, ministerio_id) e estourava o constraint.
--
-- SOLUÇÃO: soft-delete a linha do solicitante + upsert do substituto,
-- em vez de mutar o membro_id diretamente.
...
BEGIN
  ...
  -- 1. Soft-delete da linha do solicitante (em vez de mutar membro_id)
  UPDATE public.escala_membros
     SET ativo       = false,
         removido_em = now()
   WHERE id = v_subst.escala_membro_id;

  -- 2. Upsert do substituto: se já existia linha (ativo ou não), reativa e confirma
  ...
  -- 3. Marca substituição como aprovada
