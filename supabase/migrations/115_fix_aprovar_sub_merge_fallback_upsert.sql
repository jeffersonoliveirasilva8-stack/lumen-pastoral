-- Migration 115 — coord_aprovar_substituicao: merge do fallback (098) + upsert (114)
--
-- Migration 114 sobrescreveu 098 e perdeu o fallback via profiles.
-- Esta migration combina as duas correções:
--   • fallback: coordenadores sem registro em membros usam profiles.paroquia_id
--   • upsert:   evita violação de unique constraint ao aprovar substituição
...
BEGIN
  ...
  -- Localiza coordenador em membros (auth_user_id ou e-mail)
  ...
  -- Fallback: coordenadores sem vínculo em membros → usa profiles
  ...
  -- Soft-delete da linha do solicitante (evita mutar membro_id diretamente,
  -- o que causava violação de unique constraint quando o substituto já existia)
  UPDATE public.escala_membros
     SET ativo       = false,
         removido_em = now()
   WHERE id = v_subst.escala_membro_id;

  -- Upsert do substituto: reativa linha existente ou cria nova sem duplicata
  ...
  VALUES
    (v_em.escala_id, v_subst.substituto_id, v_em.ministerio_id,
     'confirmado', true, NULL, 'substituicao')
