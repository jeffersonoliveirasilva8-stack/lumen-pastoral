-- migration 065 — Notificação push para membros elegíveis ao abrir substituição
--
-- Problema: quando um membro recusa ou solicita substituição, a vaga é criada mas
-- nenhum outro membro recebe notificação in-app nem e-mail automático.
-- A edge function "notificar-substituicao" já existe e envia e-mails, mas nunca
-- era chamada. Os membros só descobriam a vaga entrando manualmente na tela.
--
-- Solução:
--   1. portal_recusar_escala — ao criar substituição:
--        • INSERT notificacoes (destinatario_id) para cada elegível (mesmo ministério,
--          ativo, sem conflito na data, sem indisponibilidade)
--        • PERFORM net.http_post → edge fn notificar-substituicao (e-mail)
--   2. portal_solicitar_substituicao — mesmo comportamento
--
-- "Elegível" = mesma função (ministerio_id do escala_membro) + ativo + paróquia +
--              não é o solicitante + não está escalado nesta escala nesta função +
--              sem indisponibilidade na data da escala
...
BEGIN
  ...
  -- Busca nome do ministério para notificação
  SELECT nome INTO v_min_nome FROM public.ministerios WHERE id = v_ministerio_id;

  -- Notifica membros elegíveis in-app (mesma função, ativo, sem conflito)
  IF v_subst_id IS NOT NULL AND v_ministerio_id IS NOT NULL THEN
    FOR v_elegivel IN
      ...
      WHERE mm.ministerio_id = v_ministerio_id
        ...
        AND m.ativo          = true
        AND mm.membro_id    <> v_membro_id          -- não notifica o próprio recusante
        -- não está já escalado nesta escala nesta função
        AND NOT EXISTS (
        ...
        -- sem indisponibilidade registrada na data
        AND NOT EXISTS (
    ...
    -- Dispara edge function para envio de e-mail (fire-and-forget via pg_net)
    BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
      NULL; -- falha no e-mail não deve bloquear o fluxo principal
    END;
  END IF;
...
BEGIN
  ...
  -- Gate: substituicao_ativa em paroquia_config_escalas
  ...
  -- Busca nome do ministério
  SELECT nome INTO v_min_nome FROM public.ministerios WHERE id = v_ministerio_id;

  -- Notifica membros elegíveis in-app
