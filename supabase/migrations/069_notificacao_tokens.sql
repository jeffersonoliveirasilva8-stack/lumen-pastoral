-- migration 069 — Tokens de uso único para chamadas internas da edge function
--
-- Problema com migration 068: app.service_role_key requer superuser para SET,
-- que o Supabase SQL Editor não possui.
--
-- Solução: cada RPC gera um UUID aleatório de curta duração (5 min) antes de
-- chamar net.http_post. A edge function valida o token via service-role client,
-- consome-o (DELETE) e só então processa. Token inválido, expirado ou reutilizado
-- resulta em 401 imediato. A anon key pública não funciona aqui.
--
-- Sem superuser. Sem segredos hardcodados. Fail-closed.

-- ── Tabela de tokens de uso único ───────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.notificacao_tokens (
  token           UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
  substituicao_id UUID        NOT NULL
    REFERENCES public.substituicoes(id) ON DELETE CASCADE,
  expires_at      TIMESTAMPTZ NOT NULL DEFAULT now() + INTERVAL '5 minutes',
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- RLS: apenas o service_role da edge function pode ler/deletar
ALTER TABLE public.notificacao_tokens ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "deny_all_non_service" ON public.notificacao_tokens;
CREATE POLICY "deny_all_non_service" ON public.notificacao_tokens USING (false);

-- Índice para expiração + limpeza
CREATE INDEX IF NOT EXISTS notificacao_tokens_expires_idx
  ON public.notificacao_tokens(expires_at);

-- Limpeza automática de tokens expirados (roda a cada hora via pg_cron)
SELECT cron.schedule(
  'limpar-tokens-notificacao',
  '0 * * * *',
  $$ DELETE FROM public.notificacao_tokens WHERE expires_at < now() $$
);

-- ── portal_recusar_escala — gera token antes de chamar edge fn ──────────────
...
BEGIN
  ...
  IF v_subst_id IS NOT NULL AND v_ministerio_id IS NOT NULL THEN
    ...
    -- Gera token de uso único (expira em 5 min) para autenticar a edge function
...
-- ── portal_solicitar_substituicao — gera token antes de chamar edge fn ───────
