-- migration 075 — Muda header do token de Authorization para X-One-Time-Token
--
-- Causa raiz: o gateway do Supabase valida o header Authorization como JWT
-- antes de passar a requisição para a edge function. Um UUID não é um JWT
-- válido, então a requisição era rejeitada com 401 antes do código executar.
-- Fix: enviar o token em X-One-Time-Token (header customizado ignorado pelo
-- gateway) e desabilitar verify_jwt via config.toml na edge function.
