-- Migration 092 — Config centralizada via funções helper (sem ALTER DATABASE)
--
-- ALTER DATABASE requer superuser (bloqueado no Supabase Cloud).
-- Alternativa: duas funções IMMUTABLE com SECURITY DEFINER que centralizam
-- URL e anon key. Para rotacionar, basta CREATE OR REPLACE no SQL Editor:
--
--   CREATE OR REPLACE FUNCTION public._cfg_anon_key()
--   RETURNS TEXT LANGUAGE sql IMMUTABLE SECURITY DEFINER
--   SET search_path = public AS $$
--     SELECT 'nova_chave_aqui';
--   $$;
--
-- As funções têm REVOKE ALL FROM PUBLIC — nenhum membro autenticado consegue
-- chamá-las diretamente. Apenas as funções internas (postgres role) as usam.
--
-- IDEMPOTENTE: sim

-- ── Funções de configuração ──────────────────────────────────────────────────

CREATE OR REPLACE FUNCTION public._cfg_supabase_url()
RETURNS TEXT
LANGUAGE sql
IMMUTABLE
SECURITY DEFINER
...
AS $$
  SELECT 'https://cusuoggmlhtvrclrzvfr.supabase.co';
$$;

REVOKE ALL  ON FUNCTION public._cfg_supabase_url()     FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public._cfg_supabase_url()   TO postgres;
...
SET search_path = public
AS $$
  SELECT 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImN1c3VvZ2dtbGh0dnJjbHJ6dmZyIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzkyNzk1OTUsImV4cCI6MjA5NDg1NTU5NX0.14p2eZ2SLHYLlJ5mWsOyxAR_cGNbxHSZ9E9kmEbuI5I';
$$;

REVOKE ALL  ON FUNCTION public._cfg_anon_key()         FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public._cfg_anon_key()       TO postgres;

-- ── _edge_fn_url: usa _cfg_supabase_url() em vez de current_setting ──────────
...
SET search_path = public
AS $$
  SELECT public._cfg_supabase_url() || '/functions/v1/' || p_fn;
$$;
