-- migration 076 — Usar anon key no Authorization + token no X-One-Time-Token
--
-- Causa raiz: o gateway do Supabase valida o JWT no header Authorization mesmo
-- com config.toml verify_jwt=false (o config.toml por função pode não ser
-- suportado na versão do CLI em uso).
-- Fix definitivo: passar o anon key público no Authorization (o gateway aceita)
-- e o token de uso único em X-One-Time-Token (validado pelo código da função).
-- O anon key não concede acesso ao banco — a segurança vem do token único.
...
DECLARE
  ...
  v_elegivel      RECORD;
  v_token         UUID;
  v_anon_key      TEXT := 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImN1c3VvZ2dtbGh0dnJjbHJ6dmZyIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzkyNzk1OTUsImV4cCI6MjA5NDg1NTU5NX0.14p2eZ2SLHYLlJ5mWsOyxAR_cGNbxHSZ9E9kmEbuI5I';
BEGIN
