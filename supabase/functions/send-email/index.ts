...
function tEscalaCancelada(nome: string, paroquia: string, escalaTitulo: string, escalaData: string, escalaHora: string, ministerioNome: string, siteUrl: string): string {
  const sn = htmlSafe(nome); const sp = htmlSafe(paroquia); const st = htmlSafe(escalaTitulo); const sf = htmlSafe(ministerioNome);
  ...
  const portalUrl = `${siteUrl}/portal-membro/home`;
  const body = `
    <h1>Escala cancelada &#10060;</h1>
    <p>Ol&aacute;, <span class="hi">${sn}</span>!</p>
    <p>Informamos que a escala abaixo foi <strong>cancelada</strong> pela coordena&ccedil;&atilde;o da <span class="hi">${sp}</span>.</p>
    <table style="width:100%;border-collapse:collapse;margin:16px 0;">
      <tr><td style="padding:6px 0;color:#888;font-size:13px;width:100px;">Escala</td><td style="padding:6px 0;font-weight:600;color:#111;">${st}</td></tr>
    ...
    <p>Voc&ecirc; <strong>n&atilde;o precisa comparecer</strong> nesta data. Em caso de d&uacute;vidas, entre em contato com a coordena&ccedil;&atilde;o.</p>
    <div class="bw"><a href="${portalUrl}" class="btn">Acessar o portal &rarr;</a></div>`;
  return baseLayout(paroquia, body, siteUrl);
}
